...
| FR-002-AC-9 | A host whose transcript is written during the session has it located after the session ends, by the transcript's own record of the scenario working directory rather than by modification time, and its usage and operation counts are reported instead of `unavailable`. | Unit         |
